...
describe ChunkyPNG::Color do
  ...
  describe '#from_cmyk' do
    it "should convert basic colors to RGB" do
      expect(from_cmyk(0x00, 0x00, 0x00, 0xff)).to eql 0x000000ff
      expect(from_cmyk(0x00, 0x00, 0x00, 0x00)).to eql 0xffffffff
      expect(from_cmyk(0x00, 0xff, 0xff, 0x00)).to eql 0xff0000ff
      expect(from_cmyk(0xff, 0x00, 0xff, 0x00)).to eql 0x00ff00ff
      expect(from_cmyk(0xff, 0xff, 0x00, 0x00)).to eql 0x0000ffff
      expect(from_cmyk(0xff, 0x00, 0x00, 0x00)).to eql 0x00ffffff
      expect(from_cmyk(0x00, 0xff, 0x00, 0x00)).to eql 0xff00ffff
      expect(from_cmyk(0x00, 0x00, 0xff, 0x00)).to eql 0xffff00ff
    end

    it "should convert blended colors to RGB" do
      expect(from_cmyk(0x17, 0x73, 0x75, 0xb5)).to eql 0x432928ff
      expect(from_cmyk(0xdb, 0x29, 0xb3, 0x5c)).to eql 0x178931ff
    end

    it "should include an alpha component if optional alpha argument is given" do
      expect(from_cmyk(0x00, 0x00, 0x00, 0xff, 0x00)).to eql 0x00000000
      expect(from_cmyk(0x00, 0x00, 0x00, 0xff, 0xaa)).to eql 0x000000aa
      expect(from_cmyk(0x00, 0x00, 0x00, 0xff, 0xff)).to eql 0x000000ff
    end
  end

  describe '#to_cmyk' do
    it "should convert basic colors to CMYK" do
      expect(to_cmyk(0x000000ff)).to eql [0x00, 0x00, 0x00, 0xff]
      expect(to_cmyk(0xffffffff)).to eql [0x00, 0x00, 0x00, 0x00]
      expect(to_cmyk(0xff0000ff)).to eql [0x00, 0xff, 0xff, 0x00]
      expect(to_cmyk(0x00ff00ff)).to eql [0xff, 0x00, 0xff, 0x00]
      expect(to_cmyk(0x0000ffff)).to eql [0xff, 0xff, 0x00, 0x00]
      expect(to_cmyk(0x00ffffff)).to eql [0xff, 0x00, 0x00, 0x00]
      expect(to_cmyk(0xff00ffff)).to eql [0x00, 0xff, 0x00, 0x00]
      expect(to_cmyk(0xffff00ff)).to eql [0x00, 0x00, 0xff, 0x00]
    end

    it "should convert blended colors to CMYK" do
      expect(to_cmyk(0x432928ff)).to eql [0x00, 0x63, 0x67, 0xbc]
      expect(to_cmyk(0x178931ff)).to eql [0xd4, 0x00, 0xa4, 0x76]
    end

    it "should include an alpha component if request" do
      expect(to_cmyk(0x00000000, true)).to eql [0x00, 0x00, 0x00, 0xff, 0x00]
      expect(to_cmyk(0x000000aa, true)).to eql [0x00, 0x00, 0x00, 0xff, 0xaa]
      expect(to_cmyk(0x000000ff, true)).to eql [0x00, 0x00, 0x00, 0xff, 0xff]
    end
  end
